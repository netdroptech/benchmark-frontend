  ...
  return (
    <AuthShell>
          ...
          <div key={i} className="flex items-center flex-1">
            <div className="flex flex-col items-center flex-shrink-0">
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all"
                style={
                  i < step
                    ? { background: 'rgba(34,197,94,0.2)',  border: '1.5px solid #4ade80', color: '#4ade80' }
                    : i === step
                    ? { background: 'rgba(34,197,94,0.15)', border: '1.5px solid #22c55e', color: '#86efac' }
                    : { background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', color: 'hsl(240 5% 45%)' }
                }>
                {i < step ? <Check size={13} /> : i + 1}
              </div>
              <span className="text-[10px] mt-1 font-medium"
                style={{ color: i === step ? '#86efac' : 'hsl(240 5% 45%)' }}>
                {s.label}
              </span>
            </div>
            {i < STEPS.length - 1 && (
              <div className="flex-1 h-px mx-2 mt-[-12px]"
                style={{ background: i < step ? 'rgba(34,197,94,0.4)' : 'rgba(255,255,255,0.08)' }} />
            )}
          </div>
        ))}
      </div>
        ...
        <div className="space-y-4">
          ...
          <label className="flex items-start gap-3 cursor-pointer">
            <div
              ...
              {form.agree && <Check size={11} style={{ color: '#4ade80' }} />}
            </div>
            <span className="text-xs leading-relaxed" style={{ color: 'hsl(240 5% 60%)' }}>
              I agree to the{' '}
              <Link to="/terms" target="_blank" rel="noopener noreferrer" className="underline" style={{ color: '#4ade80' }}>Terms of Service</Link>
              {' '}and{' '}
              <Link to="/privacy" target="_blank" rel="noopener noreferrer" className="underline" style={{ color: '#4ade80' }}>Privacy Policy</Link>.
      ...
      <p className="text-center text-xs mt-6" style={{ color: 'hsl(240 5% 50%)' }}>
        ...
        <Link to="/login" className="font-medium" style={{ color: '#4ade80' }}>Sign in</Link>
      </p>
    </AuthShell>
  )
}
    ...
    <div className="min-h-screen flex items-center justify-center p-4" style={{ background: 'hsl(260 87% 2%)', position: 'relative', overflow: 'hidden' }}>
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        ...
        <div className="absolute bottom-[-20%] right-[-10%] w-[500px] h-[500px] rounded-full opacity-15"
          style={{ background: 'radial-gradient(circle,#15803d,transparent 70%)', filter: 'blur(60px)' }} />
      </div>
