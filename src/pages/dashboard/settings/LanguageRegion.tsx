    ...
    <div className="p-4 md:p-6 max-w-[760px] mx-auto overflow-x-hidden">
      ...
      <Section title="Language" sub="Choose the display language for your dashboard">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }} className="sm:grid-cols-3">
          {LANGUAGES.map(l => (
            <button key={l.code} onClick={() => setLang(l.code)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderRadius: 10, cursor: 'pointer', border: lang === l.code ? '1px solid rgba(74,222,128,0.4)' : '1px solid rgba(255,255,255,0.07)', background: lang === l.code ? 'rgba(74,222,128,0.1)' : 'rgba(255,255,255,0.02)', textAlign: 'left', transition: 'all 0.15s' }}>
              <span style={{ fontSize: 18 }}>{l.flag}</span>
              <span style={{ fontSize: 12, fontWeight: 600, color: lang === l.code ? '#86efac' : 'hsl(40 6% 75%)' }}>{l.label}</span>
              {lang === l.code && <CheckCircle2 size={12} style={{ color: '#4ade80', marginLeft: 'auto' }} />}
            </button>
          ))}
        </div>
      </Section>
      ...
      {/* Preview */}
      <div style={{ background: 'rgba(74,222,128,0.06)', border: '1px solid rgba(74,222,128,0.15)', borderRadius: 12, padding: '16px 20px', marginBottom: 24 }}>
        <p style={{ fontSize: 12, fontWeight: 700, color: '#86efac', marginBottom: 10 }}>Preview</p>
