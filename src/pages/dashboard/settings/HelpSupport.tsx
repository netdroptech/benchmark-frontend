...
const CHANNELS = [
  { icon: MessageCircle, label: 'Live Chat',     sub: 'Average reply: 2 min',  color: '#4ade80', bg: 'rgba(74,222,128,0.1)',   cta: 'Start Chat',    available: true  },
    ...
    <div className="p-4 md:p-6 max-w-[760px] mx-auto overflow-x-hidden">
      ...
      <div style={{ background: 'hsl(260 60% 5%)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, padding: 24, marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 20 }}>
          {categories.map(c => (
            <button key={c} onClick={() => setCategory(c)} style={{ padding: '5px 12px', borderRadius: 999, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: category === c ? '1px solid rgba(74,222,128,0.4)' : '1px solid rgba(255,255,255,0.08)', background: category === c ? 'rgba(74,222,128,0.15)' : 'rgba(255,255,255,0.03)', color: category === c ? '#86efac' : 'hsl(240 5% 55%)', transition: 'all 0.15s' }}>
              {c}
            </button>
          ))}
        </div>
          ...
          {filtered.map(group => (
            <div key={group.category}>
              ...
              {group.items.map(item => {
                ...
                return (
                  <div key={key} style={{ borderRadius: 10, overflow: 'hidden', marginBottom: 6, border: isOpen ? '1px solid rgba(74,222,128,0.2)' : '1px solid rgba(255,255,255,0.06)', background: isOpen ? 'rgba(74,222,128,0.04)' : 'rgba(255,255,255,0.02)' }}>
      ...
      <div style={{ background: 'hsl(260 60% 5%)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, padding: 24, marginBottom: 20 }}>
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          {[
            ...
            { icon: BookOpen, label: 'Trading Academy',     sub: 'Beginner to advanced courses',  color: '#4ade80' },
            { icon: Shield,   label: 'Security Centre',     sub: 'Best practices & safety tips',  color: '#4ade80' },
      ...
      <div style={{ background: 'hsl(260 60% 5%)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, padding: 24, marginBottom: 20 }}>
        ...
        {sent ? (
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            ...
            <p style={{ fontSize: 15, fontWeight: 700, color: '#4ade80', marginBottom: 6 }}>Message Sent!</p>
          ...
          <>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <button onClick={handleSend} disabled={!subject || !message} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 22px', borderRadius: 10, background: subject && message ? 'linear-gradient(135deg,#16a34a,#15803d)' : 'rgba(255,255,255,0.05)', border: 'none', color: subject && message ? '#fff' : 'hsl(240 5% 40%)', fontSize: 13, fontWeight: 700, cursor: subject && message ? 'pointer' : 'default', transition: 'all 0.2s' }}>
