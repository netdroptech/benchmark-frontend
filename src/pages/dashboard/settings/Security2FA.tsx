    ...
    <div className="p-4 md:p-6 max-w-[760px] mx-auto overflow-x-hidden">
      ...
      <div style={{ background: 'hsl(260 60% 5%)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, padding: 24, marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 48, height: 48, borderRadius: 12, background: twoFAEnabled ? 'rgba(74,222,128,0.12)' : 'rgba(245,158,11,0.12)', border: `1px solid ${twoFAEnabled ? 'rgba(74,222,128,0.2)' : 'rgba(245,158,11,0.2)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Shield size={22} style={{ color: twoFAEnabled ? '#4ade80' : '#f59e0b' }} />
            </div>
            <div>
              <p style={{ fontSize: 14, fontWeight: 700, color: 'hsl(40 10% 94%)' }}>Security Score</p>
              <p style={{ fontSize: 12, color: twoFAEnabled ? '#4ade80' : '#f59e0b' }}>{twoFAEnabled ? 'Strong — All protections active' : 'Fair — Enable 2FA to strengthen'}</p>
            </div>
          </div>
          <div style={{ fontSize: 32, fontWeight: 800, color: twoFAEnabled ? '#4ade80' : '#f59e0b' }}>{twoFAEnabled ? '95' : '65'}<span style={{ fontSize: 14, fontWeight: 500, color: 'hsl(240 5% 50%)' }}>/100</span></div>
        </div>
        {/* Score bar */}
        <div style={{ height: 6, borderRadius: 999, background: 'rgba(255,255,255,0.06)' }}>
          <div style={{ height: '100%', width: twoFAEnabled ? '95%' : '65%', borderRadius: 999, background: twoFAEnabled ? 'linear-gradient(90deg,#4ade80,#22c55e)' : 'linear-gradient(90deg,#f59e0b,#d97706)', transition: 'all 0.5s ease' }} />
        </div>
      </div>
      ...
      <Section title="Two-Factor Authentication" sub="Add an extra layer of security using an authenticator app">
        {!twoFAEnabled && !showSetup && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
            ...
            <button onClick={() => setShowSetup(true)} style={{ padding: '9px 18px', borderRadius: 9, background: 'linear-gradient(135deg,#16a34a,#15803d)', border: 'none', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7 }}>
        ...
        {twoFAEnabled && !showSetup && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ width: 36, height: 36, borderRadius: 9, background: 'rgba(74,222,128,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <CheckCircle2 size={16} style={{ color: '#4ade80' }} />
              </div>
              <div>
                <p style={{ fontSize: 13, fontWeight: 600, color: '#4ade80' }}>2FA is enabled</p>
        ...
        {showSetup && otpStep === 'qr' && (
          <div>
            ...
            <button onClick={() => setOtpStep('verify')} style={{ width: '100%', padding: '11px', borderRadius: 10, background: 'linear-gradient(135deg,#16a34a,#15803d)', border: 'none', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
        ...
        {showSetup && otpStep === 'verify' && (
          <div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={handleVerify} style={{ flex: 2, padding: '11px', borderRadius: 10, background: otpCode.length === 6 ? 'linear-gradient(135deg,#16a34a,#15803d)' : 'rgba(255,255,255,0.05)', border: 'none', color: otpCode.length === 6 ? '#fff' : 'hsl(240 5% 40%)', fontSize: 13, fontWeight: 700, cursor: otpCode.length === 6 ? 'pointer' : 'default' }}>
                Verify & Activate
              </button>
            </div>
          </div>
        ...
        {showSetup && otpStep === 'done' && (
          <div style={{ textAlign: 'center', padding: '8px 0' }}>
            ...
            <p style={{ fontSize: 15, fontWeight: 700, color: '#4ade80', marginBottom: 6 }}>2FA Successfully Enabled!</p>
            <p style={{ fontSize: 12, color: 'hsl(240 5% 52%)', marginBottom: 20 }}>Your account is now protected with two-factor authentication.</p>
            <button onClick={() => setShowSetup(false)} style={{ padding: '10px 28px', borderRadius: 10, background: 'linear-gradient(135deg,#16a34a,#15803d)', border: 'none', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
              Done
            </button>
          </div>
        )}
      ...
      <Section title="Change Password" sub="Use a strong password that you don't use elsewhere">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 4 }}>
            <button onClick={() => { setPwSaved(true); setTimeout(() => setPwSaved(false), 3000) }} style={{ padding: '10px 22px', borderRadius: 10, background: pwSaved ? 'rgba(74,222,128,0.15)' : 'linear-gradient(135deg,#16a34a,#15803d)', border: pwSaved ? '1px solid rgba(74,222,128,0.3)' : 'none', color: pwSaved ? '#4ade80' : '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7 }}>
      ...
      <Section title="Backup Codes" sub="Use these one-time codes if you lose access to your authenticator app">
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <button onClick={() => { navigator.clipboard?.writeText(BACKUP_CODES.join('\n')); setCopiedCodes(true); setTimeout(() => setCopiedCodes(false), 2000) }} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', borderRadius: 8, background: copiedCodes ? 'rgba(74,222,128,0.1)' : 'rgba(255,255,255,0.04)', border: `1px solid ${copiedCodes ? 'rgba(74,222,128,0.2)' : 'rgba(255,255,255,0.08)'}`, color: copiedCodes ? '#4ade80' : 'hsl(240 5% 55%)', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
            <Copy size={12} /> {copiedCodes ? 'Copied!' : 'Copy All'}
          </button>
          <button style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', borderRadius: 8, background: 'rgba(74,222,128,0.12)', border: '1px solid rgba(74,222,128,0.2)', color: '#86efac', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
            Regenerate Codes
          </button>
        </div>
      </Section>
      ...
      <Section title="Active Sessions" sub="Devices currently signed into your account">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {SESSIONS.map(s => (
            <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', borderRadius: 10, background: s.current ? 'rgba(74,222,128,0.04)' : 'rgba(255,255,255,0.02)', border: `1px solid ${s.current ? 'rgba(74,222,128,0.12)' : 'rgba(255,255,255,0.06)'}`, flexWrap: 'wrap' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  {s.current && <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 999, background: 'rgba(74,222,128,0.12)', color: '#4ade80' }}>This device</span>}
                </div>
