        ...
        return (
          <div key={d.month + i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
            ...
            <div style={{
              ...
              background: isLast ? 'linear-gradient(180deg,#88fc8a,#00ff04)' : 'rgba(136,252,138,0.3)',
    ...
    <div className="p-4 md:p-6 max-w-[1400px] mx-auto overflow-x-hidden">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
        {loading
          ...
          : [
              { label: 'Total Deposited', value: `$${totalIn.toLocaleString('en-US', { minimumFractionDigits: 2 })}`,              icon: ArrowDownLeft, color: '#4ade80', bg: 'rgba(74,222,128,0.1)'  },
              ...
              { label: 'Net Balance',     value: `$${(totalIn - totalOut).toLocaleString('en-US', { minimumFractionDigits: 2 })}`, icon: TrendingUp,    color: '#4ade80', bg: 'rgba(74,222,128,0.1)'  },
            ].map(s => (
          ...
          <div className="flex gap-1 flex-wrap">
            {FILTERS.map(f => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                style={{ padding: '0.3rem 0.75rem', borderRadius: 999, fontSize: 12, fontWeight: 500, cursor: 'pointer', border: 'none', background: filter === f ? 'rgba(74,222,128,0.2)' : 'transparent', color: filter === f ? '#86efac' : 'hsl(240 5% 55%)', transition: 'all 0.15s' }}
              >
                {f}
              </button>
            ))}
