  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: 'hsl(260 87% 3%)' }}>
              ...
              {QUICK_AMOUNTS.map(a => (
                <button
                  ...
                  style={{
                    ...
                    background: amount === String(a) ? 'rgba(74,222,128,0.2)' : 'rgba(255,255,255,0.04)',
                    color: amount === String(a) ? '#86efac' : 'hsl(240 5% 55%)',
          ...
          {amount && parseFloat(amount) > 0 && (
            <div style={{
              ...
              {[
                ...
                { label: 'Direction', value: tradeType, color: tradeType === 'CALL' ? '#4ade80' : '#f87171' },
                { label: 'Amount',    value: `$${parseFloat(amount).toFixed(2)}` },
                { label: 'Duration',  value: duration },
                { label: 'Potential Payout', value: `$${(parseFloat(amount) * 1.85).toFixed(2)}`, color: '#4ade80' },
              ].map(row => (
      ...
      <style>{`
        ...
        @keyframes liveBlink { 0%,100%{opacity:1;box-shadow:0 0 6px #4ade80} 50%{opacity:0.2;box-shadow:none} }
      `}</style>
    </div>
  )
}
