...
const BOTS = [
  ...
  },
  {
    ...
    color: '#4ade80', equity: [100,101,102,101,103,104,103,105,104,106],
  ...
  },
  {
    ...
    color: '#4ade80', equity: [100,102,104,103,106,108,107,110,109,112],
  ...
  },
  {
    ...
    color: '#34d399', equity: [100,101,103,102,104,105,104,106,105,107],
  ...
  return (
    <div className="p-4 md:p-6 max-w-[1400px] mx-auto overflow-x-hidden">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '0.5rem 1rem', borderRadius: '0.6rem', background: 'linear-gradient(135deg, #88fc8a 0%, #00ff04 100%)', color: '#050505', fontSize: 13, fontWeight: 600, border: 'none', cursor: 'pointer' }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
        {[
          { label: 'Active Bots',     value: `${activeBots} / ${bots.length}`,   icon: Activity,   color: '#4ade80', bg: 'rgba(74,222,128,0.1)'   },
          { label: 'Combined Return', value: `+${totalReturn.toFixed(1)}%`,       icon: TrendingUp, color: '#4ade80', bg: 'rgba(74,222,128,0.1)'  },
            ...
            <div key={bot.id} style={{ background: 'hsl(260 60% 5%)', border: `1px solid ${isActive ? 'rgba(255,255,255,0.09)' : 'rgba(255,255,255,0.05)'}`, borderRadius: '0.875rem', padding: '1.25rem', display: 'flex', flexDirection: 'column', opacity: isActive ? 1 : 0.7, transition: 'all 0.2s' }}>
              ...
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex items-center gap-1.5">
                  <div style={{ width: 7, height: 7, borderRadius: '50%', background: isActive ? '#4ade80' : '#94a3b8', boxShadow: isActive ? '0 0 6px #4ade80' : 'none' }} />
                  <span style={{ fontSize: 11, color: isActive ? '#4ade80' : 'hsl(240 5% 50%)' }}>{isActive ? 'Active' : 'Paused'}</span>
                </div>
              </div>
              ...
              <div className="flex items-end justify-between mb-3">
                <div>
                  <p style={{ fontSize: '1.5rem', fontWeight: 800, color: isActive ? '#4ade80' : 'hsl(240 5% 55%)', letterSpacing: '-0.02em' }}>{bot.return}</p>
              ...
              <div className="flex gap-2 mt-auto">
                <button
                  ...
                  style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, padding: '0.55rem', borderRadius: '0.6rem', fontSize: 12, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s', ...(isActive ? { background: 'rgba(248,113,113,0.12)', color: '#f87171', border: '1px solid rgba(248,113,113,0.2)' } : { background: 'linear-gradient(135deg, #88fc8a 0%, #00ff04 100%)', color: '#050505', border: 'none' }) }}
                >
