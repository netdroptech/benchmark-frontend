      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 150 }} preserveAspectRatio="none">
        <defs>
          <linearGradient id="eqGrad" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%"   stopColor="#88fc8a" stopOpacity={0.22} />
            <stop offset="100%" stopColor="#88fc8a" stopOpacity={0} />
          </linearGradient>
        </defs>
        <polygon points={area} fill="url(#eqGrad)" />
        <polyline points={line} fill="none" stroke="#88fc8a" strokeWidth="2.5" strokeLinejoin="round" />
        <circle cx={pts[pts.length - 1].x} cy={pts[pts.length - 1].y} r="4" fill="#88fc8a" />
      </svg>
      ...
      {months.map(d => {
        ...
        return (
          <div key={d.label} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', gap: 4, height: '100%' }}>
            <span style={{ fontSize: 9, color: isPos ? '#4ade80' : '#f87171', fontWeight: 600 }}>
              {isPos ? '+' : ''}
              {Math.abs(d.pnl) >= 1000 ? `$${(d.pnl / 1000).toFixed(1)}k` : `$${d.pnl.toFixed(0)}`}
            </span>
            <div style={{ height: `${Math.max(pct, 4)}%`, width: '100%', borderRadius: '3px 3px 0 0', background: isPos ? 'rgba(74,222,128,0.7)' : 'rgba(248,113,113,0.7)' }} />
          ...
          : [
              {
                ...
                bg:    totalProfit >= 0 ? 'rgba(74,222,128,0.1)' : 'rgba(248,113,113,0.1)',
              },
              {
                label: 'Win Rate',
      ...
      <Card style={{ padding: '1.25rem 1.5rem', marginBottom: '1.25rem' }}>
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-1.5">
            <div style={{ width: 8, height: 2, background: '#88fc8a', borderRadius: 1 }} />
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
          ...
          {!loading && recentEntries.length > 0 && (
            <div className="flex items-center gap-3 text-xs" style={{ color: 'hsl(240 5% 50%)' }}>
              <span style={{ color: '#4ade80' }}>● {winCount} positive</span>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            ...
            <tbody>
                ...
                <tr>
                  <td colSpan={5} style={{ padding: '52px 20px', textAlign: 'center' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
                      <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'rgba(74,222,128,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <TrendingUp size={18} style={{ color: 'rgba(74,222,128,0.3)' }} />
                      </div>
              ...
              {!loading && recentEntries.map((t, i) => {
                ...
                const statusColor =
                  t.status === 'COMPLETED' ? { color: '#4ade80', bg: 'rgba(74,222,128,0.12)' }
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: '0.875rem 1.25rem' }}>
                      <span style={{ fontSize: 11, fontWeight: 500, padding: '2px 8px', borderRadius: 999, background: t.type === 'BONUS' ? 'rgba(245,158,11,0.12)' : 'rgba(74,222,128,0.12)', color: t.type === 'BONUS' ? '#f59e0b' : '#4ade80' }}>
                        {t.type === 'PROFIT' ? 'Profit' : 'Bonus'}
                      </span>
                    </td>
                    <td style={{ padding: '0.875rem 1.25rem', fontWeight: 600, color: isPos ? '#4ade80' : '#f87171', whiteSpace: 'nowrap' }}>
