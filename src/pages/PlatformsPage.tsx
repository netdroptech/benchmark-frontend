...
function HeroSection() {
  return (
    <section className="relative w-full overflow-hidden pt-6 pb-24 sm:pb-32">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[900px] h-[500px] pointer-events-none" style={{ background: 'radial-gradient(ellipse at 50% 0%, rgba(136,252,138,0.07) 0%, transparent 70%)' }} />
      <Navbar />
        ...
        <div className="grid sm:grid-cols-3 gap-5 sm:gap-6">
          {PLATFORMS.map(({ icon: Icon, name, tag, desc, features, cta, highlight }, i) => (
            <Reveal key={name} delay={i * 100}>
              <div className="liquid-glass rounded-3xl p-7 sm:p-8 flex flex-col gap-6" style={highlight ? { boxShadow: 'inset 0 1px 1px rgba(136,252,138,0.15), 0 0 0 1px rgba(136,252,138,0.15)' } : {}}>
                <div className="flex items-start justify-between">
                  <div className="w-12 h-12 rounded-2xl flex items-center justify-center shrink-0" style={{ background: 'rgba(136,252,138,0.1)', border: '1px solid rgba(136,252,138,0.2)' }}>
                    <Icon className="w-6 h-6" style={{ color: '#88fc8a' }} strokeWidth={1.5} />
                  </div>
          ...
          {FEATURES.map(({ icon: Icon, title, desc }, i) => (
            <Reveal key={title} delay={i * 80}>
              <div className="liquid-glass rounded-3xl p-6 sm:p-7 flex flex-col gap-4 hover:bg-white/[0.03] transition-colors">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ background: 'rgba(136,252,138,0.1)', border: '1px solid rgba(136,252,138,0.2)' }}>
                  <Icon className="w-5 h-5" style={{ color: '#88fc8a' }} strokeWidth={1.5} />
                </div>
                ...
                <div key={feature} className={`grid grid-cols-4 px-5 sm:px-6 py-3 sm:py-4 ${idx < COMPARISON.length - 1 ? 'border-b border-border/20' : ''} hover:bg-white/[0.02] transition-colors`}>
                  ...
                  {[web, mobile, api].map((supported, i) => (
                    <div key={i} className="flex justify-center">
                      {supported ? <span className="text-sm sm:text-base" style={{ color: '#88fc8a' }}>✓</span> : <span className="text-muted-foreground/30 text-sm sm:text-base">–</span>}
                    </div>
                  ))}
                </div>
              ))}
