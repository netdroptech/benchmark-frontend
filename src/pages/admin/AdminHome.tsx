  ...
  return (
    <div style={{ background: 'hsl(260 60% 5%)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div>
        ...
        <p style={{ fontSize: 11, marginTop: 6, display: 'flex', alignItems: 'center', gap: 4, color: subPositive === undefined ? 'hsl(240 5% 50%)' : subPositive ? '#4ade80' : '#f87171', fontWeight: 500 }}>
...
export function AdminHome() {
  ...
  const STATUS_COLORS: Record<string, { c: string; bg: string; label: string }> = {
    COMPLETED:  { c: '#4ade80', bg: 'rgba(74,222,128,0.12)',  label: 'Completed' },
  ...
  const TYPE_META: Record<string, { c: string; label: string }> = {
    ...
    REFERRAL:   { c: '#4ade80', label: 'Referral' },
  }

  // ── Loading skeleton ──
  if (loading) return (
    <div style={{ padding: '40px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
      <div style={{ width: 36, height: 36, border: '3px solid rgba(74,222,128,0.3)', borderTopColor: '#4ade80', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
  ...
  return (
    <div style={{ padding: '20px 16px 40px' }} className="md:p-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4" style={{ marginBottom: 24 }}>
        <StatCard icon={Users}           iconColor="#4ade80" iconBg="rgba(74,222,128,0.12)" label="Total Users"       value={String(stats.totalUsers)}           sub={`${stats.activeUsers} active`} />
        <StatCard icon={DollarSign}      iconColor="#4ade80" iconBg="rgba(74,222,128,0.12)"  label="Total AUM"         value={fmtUSD(stats.totalDeposited ?? 0)}  sub="Total deposited" />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4" style={{ marginBottom: 28 }}>
        <StatCard icon={Activity}    iconColor="#34d399" iconBg="rgba(52,211,153,0.12)"  label="Active Users"       value={String(stats.activeUsers)}           sub="KYC approved" />
        ...
        <StatCard icon={TrendingUp}  iconColor="#4ade80" iconBg="rgba(74,222,128,0.12)" label="Net Revenue"       value={fmtUSD(netRevenue)}                  sub="Deposits minus withdrawals" subPositive={netRevenue >= 0} />
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div style={{ gridColumn: 'span 2', background: 'hsl(260 60% 5%)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <button onClick={() => navigate('/admin/transactions')} style={{ fontSize: 12, fontWeight: 600, color: '#4ade80', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>View all <ChevronRight size={13} /></button>
          </div>
                  ...
                  {stats.recentTransactions.map(tx => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td style={{ padding: '10px 14px', fontSize: 12, fontWeight: 700, color: isOut ? '#f87171' : '#4ade80', whiteSpace: 'nowrap' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ background: 'hsl(260 60% 5%)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '16px' }}>
            ...
            {[
              { label: 'Active',    count: stats.activeUsers,                              total: stats.totalUsers || 1, color: '#4ade80' },
