...
const AVATAR_COLORS = [
  '#22c55e','#ec4899','#f59e0b','#10b981','#3b82f6',
  '#16a34a','#f97316','#06b6d4','#ef4444','#a3e635',
]
    ...
    <div className="p-4 md:p-6 max-w-[1400px] mx-auto overflow-x-hidden">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button onClick={openAdd} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '0.5rem 1rem', borderRadius: '0.6rem', background: 'linear-gradient(135deg,#88fc8a,#00ff04)', color: '#050505', fontSize: 13, fontWeight: 600, border: 'none', cursor: 'pointer' }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        {[
          { label: 'Total Traders',  value: loading ? '…' : String(traders.length), icon: Copy,      color: '#4ade80', bg: 'rgba(74,222,128,0.1)' },
          { label: 'Active Traders', value: loading ? '…' : String(active),         icon: Users,     color: '#4ade80', bg: 'rgba(74,222,128,0.1)' },
          { label: 'Avg. Win Rate',  value: loading ? '…' : `${avgWin}%`,           icon: TrendingUp,color: '#4ade80', bg: 'rgba(74,222,128,0.1)' },
        ].map(s => (
      ...
      {loading ? (
        ...
        <div style={{ padding: '60px 20px', textAlign: 'center', background: 'hsl(260 60% 5%)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '0.875rem' }}>
          ...
          <button onClick={openAdd} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '0.5rem 1.25rem', borderRadius: '0.6rem', background: 'rgba(74,222,128,0.15)', color: '#4ade80', border: '1px solid rgba(74,222,128,0.25)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
          ...
          {traders.map(t => (
            <div key={t.id} style={{ background: 'hsl(260 60% 5%)', border: `1px solid ${t.isActive ? 'rgba(255,255,255,0.09)' : 'rgba(255,255,255,0.04)'}`, borderRadius: '0.875rem', padding: '1.25rem', opacity: t.isActive ? 1 : 0.55, transition: 'opacity 0.2s' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
                ...
                {/* Toggle */}
                <button onClick={() => handleToggle(t)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: t.isActive ? '#4ade80' : 'hsl(240 5% 40%)', padding: 0, flexShrink: 0 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6, marginBottom: 10 }}>
                {[
                  ...
                  <div key={s.label} style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 8, padding: '6px 4px', textAlign: 'center' }}>
                    <p style={{ fontSize: 13, fontWeight: 700, color: '#4ade80' }}>{s.value}</p>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>
                <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 7px', borderRadius: 999, color: RISK_COLORS[t.riskLevel] ?? '#4ade80', background: `${RISK_COLORS[t.riskLevel] ?? '#4ade80'}18` }}>{t.riskLevel} Risk</span>
          ...
          <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 560, maxHeight: '90vh', overflowY: 'auto', background: 'hsl(260 87% 4%)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: '1.5rem', scrollbarWidth: 'none' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '1.25rem' }}>
              <div
                ...
                style={{ width: 90, height: 90, borderRadius: '50%', cursor: 'pointer', overflow: 'hidden', border: '2px dashed rgba(74,222,128,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.04)', position: 'relative' }}
              >
                {photoPreview ? (
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: '1.5rem' }}>
              ...
              <button onClick={handleSave} disabled={saving} style={{ flex: 1, padding: '0.6rem', borderRadius: 10, background: saving ? 'rgba(74,222,128,0.3)' : 'linear-gradient(135deg,#88fc8a,#00ff04)', color: '#050505', fontSize: 13, fontWeight: 700, border: 'none', cursor: saving ? 'not-allowed' : 'pointer' }}>
