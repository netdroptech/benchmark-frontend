...
const STATUS_STYLES: Record<string, { c: string; bg: string; label: string }> = {
  ACTIVE:    { c: '#4ade80', bg: 'rgba(74,222,128,0.12)',  label: 'Active' },
...
const PLAN_COLORS: Record<string, string> = {
  ELITE: '#f59e0b', PREMIUM: '#4ade80', STANDARD: '#60a5fa', FREE: '#94a3b8',
}
...
function avatarColor(email: string) {
  const colors = ['#4ade80','#60a5fa','#4ade80','#f59e0b','#f87171','#34d399']
  let h = 0
...
export function AdminUsers() {
  ...
  const SortIcon = ({ k }: { k: SortKey }) => sortKey === k
    ? (sortAsc ? <ChevronUp size={11} style={{ color: '#4ade80' }} /> : <ChevronDown size={11} style={{ color: '#4ade80' }} />)
                ...
                {sorted.map(u => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: '11px 14px', fontSize: 12.5, fontWeight: 700, color: '#4ade80', whiteSpace: 'nowrap' }}>${Number(u.balance).toLocaleString()}</td>
                      ...
                      <td style={{ padding: '11px 14px' }}>
                        <button onClick={e => { e.stopPropagation(); navigate(`/admin/users/${u.id}`) }}
                          style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 10px', borderRadius: 7, background: 'rgba(74,222,128,0.15)', border: '1px solid rgba(74,222,128,0.25)', color: '#86efac', fontSize: 11, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderTop: '1px solid rgba(255,255,255,0.06)', flexWrap: 'wrap', gap: 10 }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                <button key={p} onClick={() => setPage(p)} style={{ width: 30, height: 30, borderRadius: 7, border: page === p ? '1px solid rgba(74,222,128,0.4)' : '1px solid rgba(255,255,255,0.08)', background: page === p ? 'rgba(74,222,128,0.18)' : 'rgba(255,255,255,0.03)', color: page === p ? '#86efac' : 'hsl(240 5% 55%)', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
                  {p}
                </button>
              ))}
            </div>
