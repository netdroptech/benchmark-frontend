...
const STATUS_C: Record<string, { c: string; bg: string }> = { Available: { c: '#4ade80', bg: 'rgba(74,222,128,0.12)' }, AVAILABLE: { c: '#4ade80', bg: 'rgba(74,222,128,0.12)' }, Pending: { c: '#f59e0b', bg: 'rgba(245,158,11,0.12)' }, PENDING: { c: '#f59e0b', bg: 'rgba(245,158,11,0.12)' }, Sold: { c: '#f87171', bg: 'rgba(248,113,113,0.12)' }, SOLD: { c: '#f87171', bg: 'rgba(248,113,113,0.12)' } }
...
export function AdminProperties() {
  ...
  const filterBtn = (active: boolean): React.CSSProperties => ({
    ...
    color: active ? '#86efac' : 'hsl(240 5% 55%)',
  })
  ...
  return (
    <div style={{ padding: '20px 16px 60px' }} className="md:p-8">
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        ...
        <button onClick={openAdd} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '9px 18px', borderRadius: 10, background: 'linear-gradient(135deg,#16a34a,#15803d)', border: 'none', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', boxShadow: '0 6px 20px rgba(21,128,61,0.35)' }}>
      ...
      {!loading && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4" style={{ marginBottom: 22 }}>
          {[
            { label: 'Total Listings',     value: String(props.length),                              icon: Building2,  c: '#4ade80', bg: 'rgba(74,222,128,0.1)' },
            { label: 'Live on Platform',   value: String(props.filter(p => p.isListed).length),        icon: Eye,        c: '#4ade80', bg: 'rgba(74,222,128,0.1)' },
      ...
      <div style={{ background: 'hsl(260 60% 5%)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 860 }}>
            ...
            <tbody>
              {displayed.map(p => {
                const sc = STATUS_C[p.status] ?? { c: '#4ade80', bg: 'rgba(74,222,128,0.12)' }
                return (
                  <tr key={p.id} style={{ borderTop: '1px solid rgba(255,255,255,0.04)', transition: 'background 0.1s' }}
                    ...
                    {/* Type */}
                    <td style={{ padding: '11px 14px' }}>
                      <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 8px', borderRadius: 6, background: 'rgba(74,222,128,0.12)', color: '#86efac' }}>{p.type}</span>
                    </td>

                    {/* Price */}
                    <td style={{ padding: '11px 14px', fontSize: 13, fontWeight: 700, color: '#4ade80', whiteSpace: 'nowrap' }}>
                    ...
                    <td style={{ padding: '11px 14px' }}>
                      <button onClick={() => toggleListed(p.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, padding: 0 }}>
                        ...
                        <span style={{ fontSize: 10.5, fontWeight: 600, color: p.isListed ? '#4ade80' : 'hsl(240 5% 38%)' }}>{p.isListed ? 'Live' : 'Hidden'}</span>
                      </button>
                    </td>
                    ...
                    <td style={{ padding: '11px 14px' }}>
                      <div style={{ display: 'flex', gap: 5 }}>
                        ...
                        <button onClick={() => openEdit(p)} style={{ width: 28, height: 28, borderRadius: 6, background: 'rgba(74,222,128,0.12)', border: '1px solid rgba(74,222,128,0.22)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }} title="Edit">
                          <Pencil size={13} style={{ color: '#86efac' }} />
                        </button>
      ...
      {(modal === 'add' || modal === 'edit') && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 500, background: 'rgba(0,0,0,0.72)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '24px 16px', overflowY: 'auto' }}>
          <div style={{ width: '100%', maxWidth: 680, background: 'hsl(260 87% 5%)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 20, overflow: 'hidden', animation: 'propDropIn 0.2s ease', marginBottom: 24 }}>
            ...
            {/* Modal header */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 22px', borderBottom: '1px solid rgba(255,255,255,0.07)', background: 'rgba(74,222,128,0.06)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 34, height: 34, borderRadius: 9, background: 'linear-gradient(135deg,#16a34a,#15803d)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{ padding: '22px 22px 26px', overflowY: 'auto', maxHeight: 'calc(90vh - 80px)' }}>
              ...
              <F label="Property Image">
                <div
                  ...
                  style={{ height: 160, borderRadius: 12, border: '2px dashed rgba(74,222,128,0.3)', background: imgPreview ? 'transparent' : 'rgba(74,222,128,0.04)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', overflow: 'hidden', position: 'relative', transition: 'border-color 0.15s' }}
                ...
                >
                  {imgPreview
                    ...
                    : <div style={{ textAlign: 'center' }}>
                        <ImagePlus size={28} style={{ color: 'rgba(74,222,128,0.5)', margin: '0 auto 8px' }} />
                        <p style={{ fontSize: 13, fontWeight: 600, color: '#4ade80' }}>Click to upload image</p>
              ...
              <F label="Gallery Images">
                <div style={{ border: '1px solid rgba(255,255,255,0.09)', borderRadius: 10, padding: 12 }}>
                  <button type="button" onClick={() => galleryRef.current?.click()} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 12px', borderRadius: 8, background: 'rgba(74,222,128,0.1)', border: '1px solid rgba(74,222,128,0.2)', color: '#86efac', fontSize: 12, fontWeight: 600, cursor: 'pointer', width: '100%', justifyContent: 'center' }}>
              ...
              <F label="Tags">
                <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>
                  {form.tags.map(t => (
                    <span key={t} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 11, fontWeight: 600, padding: '3px 9px', borderRadius: 6, background: 'rgba(74,222,128,0.15)', border: '1px solid rgba(74,222,128,0.25)', color: '#86efac' }}>
                      {t}
                      <button onClick={() => removeTag(t)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#4ade80', padding: 0, display: 'flex', lineHeight: 1 }}><X size={10} /></button>
                    </span>
                  ))}
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button onClick={addTag} style={{ padding: '0 14px', borderRadius: 8, background: 'rgba(74,222,128,0.15)', border: '1px solid rgba(74,222,128,0.25)', color: '#86efac', fontSize: 12, fontWeight: 700, cursor: 'pointer', flexShrink: 0 }}>
                    <Tag size={13} />
                  </button>
                </div>
              </F>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', borderRadius: 10, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', marginBottom: 20 }}>
                ...
                <button onClick={() => setForm(f => ({ ...f, isListed: !f.isListed }))} style={{ background: 'none', border: 'none', cursor: 'pointer' }}>
                  {form.isListed
                    ? <ToggleRight size={32} style={{ color: '#4ade80' }} />
                    : <ToggleLeft  size={32} style={{ color: 'hsl(240 5% 38%)' }} />}
                </button>
              </div>

              {/* Save */}
              <button onClick={handleSave} disabled={!form.title || !form.city || saving} style={{ width: '100%', height: 46, borderRadius: 11, background: saved ? 'rgba(74,222,128,0.15)' : saving ? 'rgba(22,163,74,0.6)' : (!form.title || !form.city) ? 'rgba(255,255,255,0.05)' : 'linear-gradient(135deg,#16a34a,#15803d)', border: saved ? '1px solid rgba(74,222,128,0.3)' : 'none', color: saved ? '#4ade80' : (!form.title || !form.city) ? 'hsl(240 5% 40%)' : '#fff', fontSize: 14, fontWeight: 700, cursor: (!form.title || !form.city || saving) ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all 0.2s', opacity: saving ? 0.8 : 1 }}>
          ...
          <div style={{ width: '100%', maxWidth: 520, background: 'hsl(260 87% 5%)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 20, overflow: 'hidden', animation: 'propDropIn 0.2s ease' }}>
            ...
            <div style={{ padding: '20px 22px 26px' }}>
              ...
              <div className="grid grid-cols-3 gap-3" style={{ marginBottom: 14 }}>
                {[
                  { label: 'Price', value: selected.price >= 1e6 ? `$${parseFloat((selected.price/1e6).toFixed(2))}M` : `$${(selected.price/1000).toFixed(0)}K`, c: '#4ade80' },
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 16 }}>
                {selected.tags.map(t => (
                  <span key={t} style={{ fontSize: 11, fontWeight: 600, padding: '3px 9px', borderRadius: 6, background: 'rgba(74,222,128,0.12)', color: '#86efac' }}>{t}</span>
                ))}
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => { setModal(null); setTimeout(() => openEdit(selected), 50) }} style={{ flex: 1, padding: '10px', borderRadius: 9, background: 'linear-gradient(135deg,#16a34a,#15803d)', border: 'none', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
