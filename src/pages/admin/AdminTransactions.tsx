...
const TYPE_META: Record<string, { c: string; bg: string; label: string }> = {
  DEPOSIT:    { c: '#4ade80', bg: 'rgba(74,222,128,0.12)',  label: 'Deposit'    },
...
const STATUS_META: Record<string, { c: string; bg: string; label: string; icon: React.ElementType }> = {
  COMPLETED:  { c: '#4ade80', bg: 'rgba(74,222,128,0.12)',  label: 'Completed',  icon: CheckCircle2 },
...
export function AdminTransactions() {
  ...
  return (
    <div style={{ padding: '20px 16px 40px' }} className="md:p-8">
      ...
      {/* Action message */}
      {actionMsg && (
        <div style={{ marginBottom: 16, padding: '11px 16px', borderRadius: 10, background: actionMsg.type === 'success' ? 'rgba(74,222,128,0.08)' : 'rgba(248,113,113,0.08)', border: `1px solid ${actionMsg.type === 'success' ? 'rgba(74,222,128,0.2)' : 'rgba(248,113,113,0.2)'}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4" style={{ marginBottom: 20 }}>
        {[
          { label: 'Total Transactions', value: String(total),                           c: '#4ade80' },
          { label: 'Deposits (Done)',    value: `$${totalDeposits.toLocaleString()}`,    c: '#4ade80' },
      ...
      <div style={{ background: 'hsl(260 60% 5%)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, overflow: 'hidden' }}>
        {loading ? (
          <div style={{ padding: '60px 20px', textAlign: 'center' }}>
            <div style={{ width: 28, height: 28, border: '3px solid rgba(74,222,128,0.3)', borderTopColor: '#4ade80', borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto 12px' }} />
                ...
                {filtered.map(tx => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: '11px 14px', fontSize: 12.5, fontWeight: 700, color: isOut ? '#f87171' : '#4ade80', whiteSpace: 'nowrap' }}>
                          ...
                          {(tx.status === 'PENDING' || tx.status === 'PROCESSING') && (
                            <>
                              <button onClick={() => approve(tx.id)} disabled={busy}
                                style={{ padding: '4px 9px', borderRadius: 6, background: busy ? 'rgba(255,255,255,0.04)' : 'rgba(74,222,128,0.12)', border: '1px solid rgba(74,222,128,0.25)', color: busy ? 'hsl(240 5% 40%)' : '#4ade80', fontSize: 11, fontWeight: 700, cursor: busy ? 'not-allowed' : 'pointer' }}>
                                {busy ? '…' : '✓ Approve'}
                              </button>
            ...
            <div style={{ display: 'flex', gap: 5 }}>
              {Array.from({ length: Math.min(totalPages, 8) }, (_, i) => i + 1).map(p => (
                <button key={p} onClick={() => setPage(p)}
                  style={{ width: 28, height: 28, borderRadius: 6, border: page === p ? '1px solid rgba(74,222,128,0.4)' : '1px solid rgba(255,255,255,0.08)', background: page === p ? 'rgba(74,222,128,0.18)' : 'rgba(255,255,255,0.03)', color: page === p ? '#86efac' : 'hsl(240 5% 55%)', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>{p}</button>
              ))}
            </div>
          </div>
        )}
