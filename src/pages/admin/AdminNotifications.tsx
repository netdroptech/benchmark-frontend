...
const TYPE_CONFIG = {
  ...
  SUCCESS: { label: 'Success', icon: CheckCircle,   color: '#4ade80', bg: 'rgba(74,222,128,0.1)',  border: 'rgba(74,222,128,0.3)'  },
    ...
    <div style={{ padding: '24px 20px 60px', maxWidth: 740, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
          <div style={{ width: 32, height: 32, borderRadius: 8, background: 'rgba(74,222,128,0.12)', border: '1px solid rgba(74,222,128,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          {([false, true] as const).map(bcast => (
            <button
              ...
              style={{
                ...
                background: broadcastMode === bcast ? 'rgba(74,222,128,0.1)' : 'rgba(255,255,255,0.03)',
                border:     broadcastMode === bcast ? '1px solid rgba(74,222,128,0.3)' : '1px solid rgba(255,255,255,0.08)',
                color:      broadcastMode === bcast ? '#4ade80' : 'hsl(240 5% 55%)',
        ...
        {!broadcastMode ? (
          ...
          <div ref={searchRef} style={{ position: 'relative' }}>
            <div style={{ position: 'relative' }}>
              ...
              />
              {searching && <Loader2 size={13} style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', color: '#4ade80', animation: 'spin 1s linear infinite' }} />}
            </div>
            ...
            {dropOpen && results.length > 0 && (
              <div style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, right: 0, zIndex: 50, borderRadius: 8, background: 'hsl(260 87% 5%)', border: '1px solid rgba(255,255,255,0.1)', overflow: 'hidden', boxShadow: '0 12px 32px rgba(0,0,0,0.5)' }}>
                {results.map(u => (
                  ...
                  >
                    <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'rgba(74,222,128,0.12)', border: '1px solid rgba(74,222,128,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    ...
                    <span style={{ marginLeft: 'auto', fontSize: 10, padding: '2px 6px', borderRadius: 999, background: u.status === 'ACTIVE' ? 'rgba(74,222,128,0.1)' : 'rgba(245,158,11,0.1)', color: u.status === 'ACTIVE' ? '#4ade80' : '#f59e0b', fontWeight: 600 }}>
                      {u.status}
                    </span>
                  </button>
                ))}
            ...
            {/* Selected user chip */}
            {selected && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8, padding: '8px 10px', borderRadius: 8, background: 'rgba(74,222,128,0.06)', border: '1px solid rgba(74,222,128,0.2)' }}>
                <div style={{ width: 24, height: 24, borderRadius: '50%', background: 'rgba(74,222,128,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <User size={11} color="#4ade80" />
                </div>
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: 12, fontWeight: 600, color: '#4ade80', margin: 0 }}>{selected.firstName} {selected.lastName}</p>
          ...
          <div>
            ...
            <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
              {loadingBroadcast
                ? <Loader2 size={12} style={{ color: '#4ade80', animation: 'spin 1s linear infinite' }} />
                : <span style={{ fontSize: 11, color: '#4ade80', fontWeight: 600 }}>{broadcastList.length}</span>
              }
      ...
      {/* Feedback */}
      {success && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', borderRadius: 8, background: 'rgba(74,222,128,0.08)', border: '1px solid rgba(74,222,128,0.2)', marginBottom: 14 }}>
      ...
      <button
        onClick={handleSend}
        disabled={sending}
        style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, width: '100%', padding: '0.875rem', borderRadius: 10, background: 'linear-gradient(135deg, #88fc8a 0%, #00ff04 100%)', color: '#050505', fontSize: 14, fontWeight: 700, border: 'none', cursor: sending ? 'not-allowed' : 'pointer', opacity: sending ? 0.7 : 1, boxShadow: '0 4px 20px rgba(136,252,138,0.2)' }}
      >
        {sending
