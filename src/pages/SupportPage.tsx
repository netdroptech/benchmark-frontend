...
function HeroSection() {
  ...
  return (
    <section className="relative w-full overflow-hidden pt-6 pb-20 sm:pb-28">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[700px] h-[400px] pointer-events-none" style={{ background: 'radial-gradient(ellipse at 50% 0%, rgba(136,252,138,0.07) 0%, transparent 70%)' }} />
      <Navbar />
          ...
          {CATEGORIES.map(({ icon: Icon, title, desc, count }, i) => (
            <Reveal key={title} delay={i * 80}>
              <button className={cn('liquid-glass rounded-3xl p-6 sm:p-7 flex items-start gap-4 sm:gap-5 text-left w-full', 'hover:bg-white/[0.04] transition-colors')}>
                <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl flex items-center justify-center shrink-0 mt-0.5" style={{ background: 'rgba(136,252,138,0.08)', border: '1px solid rgba(136,252,138,0.18)' }}>
          ...
          {CONTACT.map(({ icon: Icon, title, desc, action, highlight }, i) => (
            <Reveal key={title} delay={i * 100}>
              <div className={cn('liquid-glass rounded-3xl p-7 sm:p-8 flex flex-col gap-5', highlight && 'ring-1 ring-white/10')}>
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center shrink-0" style={highlight ? { background: 'var(--primary-gradient)' } : { background: 'rgba(136,252,138,0.08)', border: '1px solid rgba(136,252,138,0.15)' }}>
                  <Icon className="w-5 h-5" style={{ color: highlight ? '#050505' : '#88fc8a' }} strokeWidth={1.8} />
                </div>
